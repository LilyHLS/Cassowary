...
Ratio residual deviance / ddl $\phi$ : It allows us to estimate whether the actual variance of the observations is not too far from the theoretical variance : $\phi = \frac{\text{variance observed}}{\text{theoretical variance}} = \frac{residual deviance}{number of degrees of freedom}$
...
remettre le modèle lm ? Vérifier la normalité des résidus de la transformation. Vérifier la corrélation spatiale des résidus (Moran, DHARMa si possible). Vérifier l’hétéroscédasticité
...
gwr.bw <- gwr.sel(log(Sightings+1) ~ Rainforest.P + Residential.P + Fruit.P + Total_Road,data = geocass, adapt = FALSE)
lmgwrD <- gwr(log(Sightings+1) ~ Rainforest.P + Residential.P + Fruit.P + Total_Road,data = geocass, bandwidth = gwr.bw, se.fit = T, hatmatrix = T)
summary(lmgwrD)
```
...
With the Poisson distribution, residuals are overfitted, probably cause to zero-inflation or outliers. So we switch to a Negative Binomial distribution. To test the zero-inflation, we also apply the function implemented in DHARMa package (@dharma_hartig) and the result of the test is significant.
