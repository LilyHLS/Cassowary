...
To answer the questions, we will first formulate a model with `Sightings` as a response variable to determine which factors influence the species abundance and then analyse the results. 
Then, we will study another model with `Strikes` as a response variable in order to identify the factors that influence the mortality of cassowary in the studied area and ways to limit it, thinking about a conservation approach.

We note that `Sightings` doesn't represent actual abundances of Cassowaries but counts made over 20 years by several people who participated in a citizen science program. The number of `Sightings` does not reflect the number of cassowary individuals, but rather the number of observations in each grid-cell of 1 km^2. Therefore, to answer the main question, we study the influence of factors on a proxy for abundance, where `Sightings` give an idea of cassowary activity and in what context they were observed most often and were therefore most abundant.
...
*Hypothesis: We assume that southern cassowary is mainly distributed near its natural habitat (Rainforest) or in areas where food is available (Fruit crop). We expect that high proportions of rainforest and fruit crops are associated with greatest cassowary abundance. We also assume that traffic strike occur when (the road is at a low distance from the centroid of the grid cell) and that risk is higher when the road connects to favorable habitat.*
...
library(GWmodel) # GWR but with Poisson distribution
...
## 1. Abundance of the cassowary

### Distribution of variables

1.  **Distribution of the response variable Sightings** $Y$ 
...
In the literature, the actual abundance of cassowaries rarely exceeds 3 individuals/km^2 (add source ?).
In some cells, some values of Sightings are up to 100 counts / km^2 but it can be normal : as explained before, Sightings don't reflect actual abundance of cassowaries but the frequency of observation by citizens.

The data will be discussed because an overcounting by citizens can be possible. Sometimes individuals car be double-counted in the context of a citizen science program. 
We could then attempt to study Y as it is and then limit the counts in certain cells to limit overdispersion and compare different methods.

2.  **Distribution of environmental variables** $X$

We assume that the environmental variables that can influence cassowary abundance are the abundance of a certain type of habitat and the presence/absence of these habitats in each cells where cassowaries are counted.
...
env <- cassowary %>%
  dplyr::select(Rainforest.P, Residential.P, Fruit.P, Other.P)
...
We can note presence of outliers in some covariates (Residential.P and Toral_Road ?)
Maybe, we could try to perform a transformation of these covariates and consider a log-transformation ?
...
There is a good balance between presence and absence for each variable.
However, studying the effect of presence/absence of fruit crop in cells and the effect of proportion of fruit crop in cell in the same model can be redundant. The same can be told for presence/absence of residential area and proportion of residential area within the grid.

3.  **Potential relationships between response variable (Sightings) and explanatory variables **
...
# Rainforest.P
plot(cassowary$Sightings~cassowary$Rainforest.P,pch=16,col='olivedrab4',xlab='Proportion of rainforest',ylab='Sightings')

#Residential.P
plot(cassowary$Sightings~cassowary$Residential.P,pch=16,col='olivedrab4',xlab='Proportion of residential area',ylab='Sightings')

# Fruit.P
plot(cassowary$Sightings~cassowary$Fruit.P,pch=16,col='olivedrab4',xlab='Proportion of fruit crop',ylab='Sightings')

#Other.P
plot(cassowary$Sightings~cassowary$Other.P,pch=16,col='olivedrab4',xlab='Proportion of wetlands and pasture',ylab='Sightings')



#Residential.01 
boxplot(cassowary$Sightings~cassowary$Residential.01,pch=16,col='olivedrab4',xlab='presence/absence of residential area',ylab='Sightings')

#Fruit.01
boxplot(cassowary$Sightings~cassowary$Fruit.01,pch=16,col='olivedrab4', xlab='presence/absence of fruit crop',ylab='Sightings')
...
4.  **Correlation between environmental variables** $X$
...
boxplot(cassowary$Rainforest.P~cassowary$Fruit.01,varwidth = TRUE, ylab = "Proportion of rainforest", xlab = "presence/absence of fruit crop", col='grey', main = "")

#Residential.P et Fruit.01
boxplot(cassowary$Residential.P~cassowary$Fruit.01,varwidth = TRUE, ylab = "Proportion of residential area", xlab = "presence/absence of fruit crop", col='grey', main = "")

#Fruit.P et Fruit.01
boxplot(cassowary$Fruit.P~cassowary$Fruit.01,varwidth = TRUE, ylab = "Proportion of fruit crop", xlab = "presence/absence of fruit crop", col='grey', main = "")

# Rainforest.P et Residential.01
boxplot(cassowary$Rainforest.P~cassowary$Residential.01,varwidth = TRUE, ylab = "Proportion de Rainforest", xlab = "presence /absence of residential area", col='grey', main = "")

#Residential.P et Residential.01
boxplot(cassowary$Residential.P~cassowary$Residential.01,varwidth = TRUE, ylab = "Proportion of residential area", xlab = "presence /absence of residential area", col='grey', main = "")

#Fruit.P et Residential.01
boxplot(cassowary$Fruit.P~cassowary$Residential.01,varwidth = TRUE, ylab = "Proportion of fruit crop", xlab = "presence /absence of residential area", col='grey', main = "")
...
5.  **Analysis of possible interactions between variables** $X$
...
plot(cassowary$Sightings~cassowary$Rainforest.P,type='n',ylab = "Sightings",xlab="Proportion of rainforest")
points(cassowary$Sightings[cassowary$Fruit.01=="0"]~cassowary$Rainforest.P[cassowary$Fruit.01=="0"],pch=16,col='darkolivegreen4') 
points(cassowary$Sightings[cassowary$Fruit.01=="1"]~cassowary$Rainforest.P[cassowary$Fruit.01=="1"],pch=17,col='darkorchid') 

# Interactions between Rainforest.P & Residential.01

plot(cassowary$Sightings~cassowary$Rainforest.P,type='n',ylab = "Sightings",xlab="Proportion of rainforest")
points(cassowary$Sightings[cassowary$Residential.01=="0"]~cassowary$Rainforest.P[cassowary$Residential.01=="0"],pch=16,col='darkolivegreen4') 
points(cassowary$Sightings[cassowary$Residential.01=="1"]~cassowary$Rainforest.P[cassowary$Residential.01=="1"],pch=17,col='darkorchid') 

# Interactions between Residential.P & Fruit.01

plot(cassowary$Sightings~cassowary$Residential.P,type='n',ylab = "Sightings",xlab="Proportion of residential area")
points(cassowary$Sightings[cassowary$Fruit.01=="0"]~cassowary$Residential.P[cassowary$Fruit.01=="0"],pch=16,col='darkolivegreen4') 
points(cassowary$Sightings[cassowary$Fruit.01=="1"]~cassowary$Residential.P[cassowary$Fruit.01=="1"],pch=17,col='darkorchid') 

# Interactions beteween Fruit.P & Residential.01

plot(cassowary$Sightings~cassowary$Fruit.P,type='n',ylab = "Sightings",xlab="Proportion of fruit crop")
points(cassowary$Sightings[cassowary$Residential.01=="0"]~cassowary$Fruit.P[cassowary$Residential.01=="0"],pch=16,col='darkolivegreen4') 
points(cassowary$Sightings[cassowary$Residential.01=="1"]~cassowary$Fruit.P[cassowary$Residential.01=="1"],pch=17,col='darkorchid') 
```
We could test interactions in the model.

6.  **Observations regarding spatial data with cartography**
...
1.  **Spatial dependency: semi-variogram**
...
Let's first try with the `Sightings` data. Maybe, remove the variables `Residential.01` and `Fruit.01` because of redundant information with the variables `Residential.P` and `Fruit.P` ? Transform the variables `Residential.01` and `Fruit.01` into qualitative ones ? Yes : maybe before the semi-variogram plot
Adele : I studied `Residential.01` and `Fruit.01` as qualitative in data exploration, and also : `Residential.01` and `Fruit.01` are redundant and correlated to `Residential.P` and `Fruit.P` so we can just keep `Residential.P` and `Fruit.P`. 
We can also remove `Other.P` : too correlated to `Rainforest.P`

Idée de modèle complet pour Poisson avec interactions (je ne sais pas si on met les interactions si ça a vraiment un sens) : 

```{r}
glm_pois <- glm(Sightings ~ Rainforest.P + Residential.P + Fruit.P + Residential.01 + Fruit.01 + 
                      Rainforest.P:Fruit.01 + Rainforest.P:Residential.01 + Residential.P:Fruit.01 + Fruit.P:Residential.01, 
                data = cassowary, family=poisson(link="log"))

drop1(glm_pois,test="Chi")
summary(glm_pois)
```

idée pour la démarche : 
ce qu'on peut faire c'est d'abord GLM complet Poisson avec tous les effets et interactions : on teste la surdispersion ensuite comme ci-dessous
si surdispersion : Negative Binomial
on construit ensuite le modèle avec Negative Binomial et on ajoute le random effect (1 | Grid_Ref) dans un glmm binomial négatif
puis on sélectionne nos variables de notre modèle avec AIC par exemple 
PS : j'ai encore rien modifié dans la suite (Adele)

Overdispersion check (scale parameter calculation) : *Overdispersion means that that residual variance is larger than expected under the fitted model. If overdispersion is present, confidence intervals will tend to be too narrow, and p-values too small, leading to inflated type I error. This phenom is most common for GLM families with constant dispersion, such as Poisson models.*
...
simout <- simulateResiduals(fittedModel = glm_pois, plot = F)
plot(simout)
```

Here, the Pearson residuals are way above 1, so there is overdispersion which can be due to outliers, model misfit, missing interactions and zero-inflation.

So we first try to change the model distribution to negative binomial : 
...
simout <- simulateResiduals(fittedModel = glm_NB, plot = F)
plot(simout)
```
...
Just in case, we can check out zeroinflation :

```{r}
testZeroInflation(simout)
```

No need to zero-inflation to the model.
...
Test for the spatial autocorrelation in the residuals with Moran Test :

```{r}
# Dharma procedure :
testSpatialAutocorrelation(simulateResiduals(glm_NB), 
                           x = cassowary$latitude, y = cassowary$longitude)
```
...
geocass <- SpatialPointsDataFrame(data.frame(cassowary[,2:3]), 
                                  data.frame(cassowary[,c("Rainforest.P","Residential.P","Other.P","Fruit.P", "Sightings")]))
gwr.bw <- gwr.sel(log(Sightings+1) ~ Rainforest.P + Residential.P + Other.P + Fruit.P,
                  data = geocass, adapt = FALSE)
lmgwrD <- gwr(log(Sightings+1) ~ Rainforest.P + Residential.P + Other.P + Fruit.P,
...
However, our results should be treated with caution as they are based on citizen science data. Furthermore, in the initial study (@dwyer_using_2016), each cassowary sighting and strike is dated, allowing for temporal analyses that take sampling bias into account.

## 2. Mortality of cassowary

### Distribution of variables

1.  **Distribution of response variable** $Y$
...
Their is some outliers values in the strike distribution (@fig-stikes_descr). Most of values stop to 5 strikes on a grid cell, but their is one with 13 strikes listed. As we don't have any information on the temporal observation, we will keep all values in the model. 
The strikes are counted, the variable follows a Poisson distribution

2.  **Distribution of explanatory variables** $X$
...
road <- cassowary %>%
  dplyr::select(Total_Road, Dresid, Dfruit, Sightings)

par(mfrow = c(2, 2))
for(i in 1:ncol(road)){
  boxplot(road[,i], col = col_transp, border = "gray30", ylab = colnames(road)[i], main = 'Boxplot')
  dotchart(road[,i], pch = 16, col = col_transp, border = "gray30", xlab = colnames(road)[i], main = 'Cleveland plot') 
  hist(road[,i], col = col_transp, border = "gray30", xlab = colnames(road)[i], main = "Histogramm")
  qqnorm(road[, i], pch = 16, col = col_transp, border = "gray30", main = "QQ plot")
  qqline(road[, i], col = 'red', lwd = 2)
}
```
We want to test which variables influence road mortality. To to this, we keep the total proportion of roads in the grid cell, but also the distance between the roads and fruit crops as well as the distance between the roads and residential area. As cassowaries have a territory of about 1km² we assume that their chances to encountering a road are high if they live close to one.
We can add `Sightings` to check if mortality strike is influenced by density of Cassowary. If we use `Sightings`, we can do a log-transformation because there are outliers in the distribution of `Sightings`.

3.  **Correlation between explanatory variables** $X$
...
There is no correlation between the $X$ variables.

### Statistical Analysis

As for the firt model, we investigate spatial dependency with the semi-variogram on variables of the problematic : **"Are there implications for strike mortality management?"**

```{r geo ref strikes}
geo_strikes <- as.geodata(SpatialPointsDataFrame(data.frame(cassowary[,2:3]),data.frame(cassowary$Strikes)))
...
```{r semivar strikes}
#| label: fig-semivario_strikes
#| fig-cap: "Empirical semi variogram for strikes mortality of cassowary and road link variables"

semivar_strikes <- variog(geo_strikes, option = "bin", max.dist = 30000, messages = FALSE)
semivar_troad<- variog(geo_troad, option = "bin", max.dist = 30000, messages = FALSE)
semivar_dresid <- variog(geo_dresid, option = "bin", max.dist = 30000, messages = FALSE)
semivar_dfruit <- variog(geo_dfruit, option = "bin", max.dist = 30000, messages = FALSE)

par(mfrow = c(2, 2))
plot(semivar_strikes, main = "Strikes mortality of cassowary")
plot(semivar_troad, main = "Proportion of road")
plot(semivar_dresid, main = "Distance of residential area to road")
plot(semivar_dfruit, main = "Distance of fruit crops to road")
```

It seems that their is no spatial dependency for variables `Total_Road` and `Dfruit`. However, the `Dresid` variable look like stationary until 30km, or with cluster?. Finally, the number of mortality strikes decrease around 13km.

```{r glm pois strikes pois}
glm_pois_strikes <- glm(Strikes ~ Total_Road + Dresid + Dfruit,
                data = cassowary, 
                family = poisson)
...
```{r glm strikes res}
testDispersion(glm_pois_strikes)
plot(simulateResiduals(glm_pois_strikes))
testZeroInflation(glm_pois_strikes)
```

Residuals are overfitting, probably cause to 0 inflation or outliers in the Poisson distribution of our $Y$ variable. As we did for the first question, we will switch to a Negative Binomial model. To test the 0 inflation we also apply the function implemented in DHARMa package and the result of the test is significant.

```{r glm strikes nb}
glm_NB_strikes <- glm.nb(Strikes ~ Total_Road + Dresid + Dfruit, 
                 data = cassowary)

drop1(glm_NB_strikes, test = "Chi")
```

```{r glm strikes nb -Dresid}
glm_NB_strikes <- glm.nb(Strikes ~ Total_Road + Dfruit, 
                 data = cassowary)
...
In this model, the variable `Dfruit` appears insignificant, but it was significant in the glm with Poisson distribution. Given that we assume that cassowaries tend to cross the road to get to where food resources are located, we retain this variable in the model. As their is no spatial dependencing in the $X$ variables `Total_Road` and `Dfruit`, we don't need to check with an other model.

```{r glm nb strikes res}
testDispersion(glm_NB_strikes)
plot(simulateResiduals(glm_NB_strikes))
testZeroInflation(glm_NB_strikes)
```

With the non binomial distribution, their is no more zero inflation. Their is no more overfitting in the residuals.

Conclusion: `Total_road` : more the proportion of road is high more there are strikes. `Dfruit` : non significant, cannot be taken in consideration, here with negative sign in the model : assume that more the distance with a fruit crops is close more their is a chance to have a strike (but here non significant with this model: make Bayesian statistic for risk analysis : better).

implication for conservation/management ? --> barrières ? passages à faune ? éloigner la route de la rainforest ?

Other discussion possibility : we don't know if the strikes are independant or if some of them where counted several times.
+ résultats potentiellement biaisés si les citoyens ne s'enfoncent pas bcp dans la forêt et restent proches de la route
