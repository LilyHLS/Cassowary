...
Using this data, compiled with the habitat variables and the distance of road location of each grid-cells (@tbl-descrva) by @dwyer_using_2016, we aim to determine **which factors influence the southern cassowary abundance. And whether there are implications for strike mortality management.**
...
However, our results should be treated with caution as they are based on citizen science data. Furthermore, in the initial study (@dwyer_using_2016), each cassowary sighting ans striking is dated, allowing for temporal analyses that take sampling bias into account.
