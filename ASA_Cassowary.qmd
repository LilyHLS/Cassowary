...
*We also assume that traffic strike occur when (the road is at a low distance from the centroid of the grid cell) and that risk is higher when the road connects to favorable habitat.*
...
#### Distribution of the response variable Sightings $Y$  {-}
...
In the literature, the actual abundance of cassowaries rarely exceeds 3 individuals/km² (add source ?).
In some cells, some values of Sightings are up to 100 counts / km², but it can be normal : as explained before, *Sightings* don't reflect actual abundance of cassowaries but the frequency of observation by citizens.
...
We assume that the environmental variables that can influence cassowary abundance are the abundance of a certain type of habitat and the presence/absence of these habitats in cells where cassowaries are counted.
...
We can note presence of outliers in some covariates (*Residential.P*)
...
```
Using graphs, it seems difficult to see relations between $Y$ and $Xs$. One explanation is that there are a lot of zeros in `Sightings`. Nevertheless, we observe that more cassowary individuals are observed when `Rainforest.P` is higher. The opposite relation is observed between Y and `Other.P`. 
`Other.P` and `Rainforest.P` could be correlated : we check correlation between $Xs$ in the following section.
...
points(Sightings[Fruit.01 == "0"] ~ Rainforest.P[Fruit.01 == "0"], 
       pch = 16, col = 'darkolivegreen4', data = cassowary) 
points(Sightings[Fruit.01 == "1"] ~ Rainforest.P[Fruit.01 == "1"], 
...
# Statistical Analysis : Sightings ~ Environmental variables
...
on construit ensuite le modèle avec Negative Binomial 
puis on sélectionne nos variables de notre modèle avec AIC par exemple 

### Poisson Distribution
As we have count data, we used **generalised linear models** (GLMs) with a **Poisson** distribution and a log link function.  
...
Adele : I studied `Residential.01` and `Fruit.01` as qualitative in data exploration,

Idée de modèle complet pour Poisson : 

```{r}
glm_pois <- glm(Sightings ~ Rainforest.P + Residential.P + Fruit.P, data = cassowary, family=poisson(link="log"))
```

```{r}
# Avec interaction 
# glm_pois <- glm(Sightings ~ Rainforest.P + Residential.P + Fruit.P + 
#                   Residential.01 + Fruit.01 + Rainforest.P:Fruit.01 +
#                   Rainforest.P:Residential.01 + Residential.P:Fruit.01 + 
#                   Fruit.P:Residential.01, 
#                 data = cassowary, family = poisson(link="log"))

glm_pois <- glm(Sightings ~ Rainforest.P + Residential.P + Fruit.P + 
                  Residential.01 + Fruit.01 + scale(Total_Road, scale = F) + 
...
8807.7/250 #ratio residual deviance / ddl : check of surdispersion
...
To account for the overdispersion of the residuals, we switch to a **negative binomial model**.
```{r}
glm_NB <- glm.nb(Sightings ~ Rainforest.P + Residential.P + Fruit.P, data = cassowary)
drop1(glm_NB, test = "Chi")
summary(glm_NB)
```

Again, we check for overdispersion :

```{r}

173.55/250 #ratio residual deviance / ddl : check of surdispersion dans le cas d'un modèle de poisson
#Le residual deviance / ddl est préféré pour vérifier la surdispersion, car il se base sur la log-vraisemblance/moins sensible aux points extrêmes.
...
Since we have spatial data, we need to check whether there is any spatial dependence between observations by looking for spatial autocorrelation in the model residuals, with Moran test.

**Getting the residuals of the NB model**
...
Test for the spatial autocorrelation in the residuals with Moran Test (Dharma package)

```{r}
# Dharma procedure :
testSpatialAutocorrelation(simulateResiduals(glm_NB), x = cassowary$longitude, y = cassowary$latitude)
```

For each point, we have a local Moran index.   
Here, the observed Moran's index is higher than the expected. This indicates a tendency towards clustering. We have significant positive autocorrelation (p-value \< 2.2e-16) : this means that two observations that are close together are more likely to be similar than two observations that are far apart.
In the case of a stationary process, the residuals are spatially random, however here the intensities and the colours of the points vary a lot which is a sign of a global non-stationarity of the relationship between *Sightings* and the explanatory variables.
...
**GWR because of the non-stationarity** :

autocorrélation spatiale positive très significative.
...
#remettre le modèle lm ?
#Vérifier la normalité des résidus de la transformation.
#Vérifier la corrélation spatiale des résidus (Moran, DHARMa si possible).
#Vérifier l’hétéroscédasticité 
...
gwr.bw <- gwr.sel(log(Sightings+1) ~ Rainforest.P + Residential.P + Fruit.P,
                  data = geocass, adapt = FALSE)
lmgwrD <- gwr(log(Sightings+1) ~ Rainforest.P + Residential.P + Fruit.P,
...
vars <- c("Rainforest.P", "Residential.P", "Fruit.P")
...
Residuals are overfitting, probably cause to 0 inflation or outliers in the Poisson distribution of our $Y$ variable. As we did for the first question, we will switch to a Negative Binomial model. To test the zero-inflation, we also apply the function implemented in DHARMa package and the result of the test is significant.
