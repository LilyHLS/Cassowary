...
Using this data, compiled with the habitat variables and the distance of road location of each grid-cells (@tbl-descrva) by @dwyer_using_2016, we aim to answer the following questions : **Which factors influence the southern cassowary abundance ? Is there any implication for strike mortality management ?**
...
*Hypothesis: We assume that southern cassowary is mainly distributed near its natural habitat (Rainforest) or in areas where food is available (Fruit crops). We expect that high proportions of rainforest and fruit crops are associated with greatest cassowary abundance. We also assume that traffic strike occur when the road is close or important in its residential area and that the risk is higher when the road connects to favorable habitat.*
...
Their are some outliers in the distribution of Y (@fig-sighting_descr), but since this is a count over a 20 years period, we will not remove them.
...
In the literature, the actual abundance of cassowaries rarely exceeds 3 individuals.km² (add source ?). In some cells, some values of Sightings are up to 100 counts.km² but it can be normal : as explained before, Sightings don't reflect actual abundance of cassowaries but the frequency of observation by citizens.

The data will be discussed because an overcounting by citizens can be possible. Sometimes individuals car be double-counted in the context of a citizen science program. We could then attempt to study Y as it is and then limit the counts in certain cells to limit overdispersion and compare different methods.
...
**a. Description of quantitative continue variables**
...
We can note presence of outliers in some covariates (Residential.P and Toral_Road ?) Maybe, we could try to perform a transformation of these covariates and consider a log-transformation ?

**b. Description of quantitative binary variables**
...
**a. Checking collinearity between continuous independent variables**
...
**b. Checking collinearity between categorical and continuous independent variables**
...
Let's first try with the `Sightings` data. Maybe, remove the variables `Residential.01` and `Fruit.01` because of redundant information with the variables `Residential.P` and `Fruit.P` ? Transform the variables `Residential.01` and `Fruit.01` into qualitative ones ? Yes : maybe before the semi-variogram plot Adele : I studied `Residential.01` and `Fruit.01` as qualitative in data exploration, and also : `Residential.01` and `Fruit.01` are redundant and correlated to `Residential.P` and `Fruit.P` so we can just keep `Residential.P` and `Fruit.P`. We can also remove `Other.P` : too correlated to `Rainforest.P`
...
idée pour la démarche : ce qu'on peut faire c'est d'abord GLM complet Poisson avec tous les effets et interactions : on teste la surdispersion ensuite comme ci-dessous si surdispersion : Negative Binomial on construit ensuite le modèle avec Negative Binomial et on ajoute le random effect (1 \| Grid_Ref) dans un glmm binomial négatif puis on sélectionne nos variables de notre modèle avec AIC par exemple PS : j'ai encore rien modifié dans la suite (Adele)
...
We want to test which variables influence road mortality. To to this, we keep the total proportion of roads in the grid cell, but also the distance between the roads and fruit crops as well as the distance between the roads and residential area. As cassowaries have a territory of about 1km² we assume that their chances to encountering a road are high if they live close to one. We can add `Sightings` to check if mortality strike is influenced by density of Cassowary. If we use `Sightings`, we can do a log+1 transformation because there are outliers in the distribution of `Sightings`.
...
There is no correlation between the $X$ variables expect for `Sighting` and `LogSighting`, but we will just keep the log+1 transformation.

### Statistical Analysis

As for the first model, in the problematic **"Are there implications for strike mortality management?"** , we begin first with a glm model. If the model is overffited, which may be due to zero inflation we change for a NB model. We then investigate the spatial autocorrelation for an noncontinuous grid.

In this model, the `Sightings` variable is transform with the log+1 transformation `LogSightings`.

comment scale

```{r glm pois strikes pois}
glm_pois_strikes <- glm(Strikes ~ scale(Total_Road) + scale(Dresid) + scale(Dfruit) + LogSightings,
                data = cassowary, 
...
```{r glm strikes res}
simresstrikes <- simulateResiduals(fittedModel = glm_pois_strikes, plot = F)
testDispersion(simresstrikes)
plot(simulateResiduals(glm_pois_strikes))
testZeroInflation(simresstrikes)
```

Residuals are overfitted but their is no issue of 0 inflated. As for the previous problematic, we switch to a NB distribution for this model.

```{r glm quasi-pois strikes pois}
glm_nb_strikes <- glm.nb(Strikes ~ scale(Total_Road) + scale(Dresid) + scale(Dfrui) + LogSightings,data = cassowary)
summary(glm_nb_strikes)
```

```{r glm.nb strikes res}
simresstrikes <- simulateResiduals(fittedModel = glm_nb_strikes, plot = F)
testDispersion(simresstrikes)
plot(simulateResiduals(glm_nb_strikes))
testZeroInflation(simresstrikes)
```

With the Negative Binomial distribution the assumption of the residuals are respected. Residuals are not anymore overfitted.
...
geo_logsightings <- as.geodata(SpatialPointsDataFrame(data.frame(cassowary[,2:3]),data.frame(cassowary$LogSightings)))
```
...
Other discussion possibility : we don't know if the strikes are independant or if some of them where counted several times. + résultats potentiellement biaisés si les citoyens ne s'enfoncent pas bcp dans la forêt et restent proches de la route
