...
We can note presence of outliers in some covariates (`Residential.P` and `Total_Road`) Maybe, we could try to perform a transformation of these covariates (scale)
...
```{r}
glm_NB <- glm.nb(Sightings ~ Rainforest.P + Residential.P + Fruit.P, 
                 data = cassowary)
