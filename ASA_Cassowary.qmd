...
1.  **Distribution of the response variable** $Y$
...
Detail for the taking to 10/20 individuals
...
Tod do : Description of the distribution: Poisson: but selection to variable? taking just the abundance of rainforest and fruit and other : presence/absence : redondance

3.  **Correlation between environmental variables** $X$

```{r correlation adundance X}
#| label: fig-corrplot
...
```{r semivar}
#| label: fig-semivario
#| fig-cap: "Empirical semi variogram for sightings of cassowary and environmental variables"
...
road <- cassowary %>%
  dplyr::select(Total_Road, Dresid, Dfruit)
...
We want to test wich variables influence road mortality. To to this, we keep the total proportion of roads in the grid cell, but also the distance to roads and fruit crops and residential area. As cassowaries have a territory of about 1km² we assume that their chances to encountering a road are high if they live close to one.

3.  Correlation between explain variables $X$
...
Conclusion: Total road : more the proportion of road is high more their is strikes. `Dfruit` : non significant but cannot take in consideration, here with negative sign in the model : assume that more the distance with a fruit crops is close more their is a chance to have a strike (but here non significant with this model: make Bayesian statistic for risk analysis : better).
