...
Maybe, we could try to perform a transformation of these covariates (scale) 
...
To account for the overdispersion of the residuals, we switch to a **negative binomial model**, with a log link.
```{r}
...
The main interesting output is the SDF : the variables (intercept) and `Rainforest.P`, `Residential.P`, `Other.P` and `Fruit.P` give the regression coefficients.   
`Intercept.se` and `Rainforest.P_se`, `Residential.P_se`, `Other.P_se` and `Fruit.P_se` give the coefficient standard errors.
