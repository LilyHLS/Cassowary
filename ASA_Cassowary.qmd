...
```
With the Poisson distribution, residuals are overfitted Residuals are overfitting, probably cause to zero-inflation or outliers in the Poisson distribution of our $Y$ variable. so we switch to a Negative Binomial distribution.
To test the zero-inflation, we also apply the function implemented in DHARMa package and the result of the test is significant.
...
```
With the negative binomial distribution, their is no more zero inflation. Their is no more overfitting in the residuals.
