...
Lorsque l'on a par exemple 100, 200 ou encore 500 casoars : ce n'est pas un comptage réel de ces casoars au km^2, mais des casoars vus plusieurs fois par plusieurs personnes au même endroit (ça peut être le même individu ou un différent, âges différents : pas de marquage-recapture) : ce n'est pas une abondance biologique.

Ecrétage des données :
10 : maximum plausible de casoars réellements distincts dans un km^2 (+ biblio et mail)
20 : permet de pas trop lisser les infos mais vis à vis de la biblio ??

Garder les valeurs telles que : peut biaiser l'autocorrélation
...
```{r}
#library(readxl)
#data_cassowary <- read_excel("data_cassowary.xlsx")
...
#conversion en numérique des variables en chaîne de caractères (pour le excel)
#data_cassowary$Rainforest.P <- as.numeric(data_cassowary$Rainforest.P)
#data_cassowary$Residential.P <- as.numeric(data_cassowary$Residential.P)
#data_cassowary$Fruit.P <- as.numeric(data_cassowary$Fruit.P)
#data_cassowary$Other.P <- as.numeric(data_cassowary$Other.P)
#data_cassowary$Total_Road <- as.numeric(data_cassowary$Total_Road)
#data_cassowary$Dresid <-as.numeric(data_cassowary$Dresid)
#data_cassowary$Dfruit <-as.numeric(data_cassowary$Dfruit)
...
#borner/écrétage 
```{r}
data_cassowary$Sightings <- pmin(data_cassowary$Sightings, 10) #bornage à 10
#data_cassowary$Sightings <- pmin(data_cassowary$Sightings, 20) #bornage à 20 : toutes les valeurs de Sightings supérieures à 20 sont ramenées à 20
table(data_cassowary$Sightings)
summary(data_cassowary$Sightings)
```

Borner à 10 :
par rapport à la biblio, souvent moins de 3 individus/km^2
ici on est sur des patchs d'un km^2 
bcp de doublons, surcomptage de certains individus

Borner à 20 : peut-être moins strict que 10 niveau écrétage 

#on refait l'exploration du Y Sightings avec écrétage 
```{r}
par(mfrow=c(2,2))
# Boxplot
boxplot(data_cassowary$Sightings,col='dodgerblue3',ylab='Visual contact of cassowary')
...
la réponse étant un comptage/abondance : nombre de casoars 
GLM Poisson. 
Si on constate un problème de surdispersion à l'issue de cette modélisation : GLM Binomiale Négative.
